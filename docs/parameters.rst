...
For polarimetric imaging with Stokes I held fixed, use ``[visQ, visU]``.
...
See also :doc:`user_guide` block 8.
...
See also :doc:`user_guide` block 7.
...
per frame) or from the zero-baseline flux (static imaging).
...
See also :doc:`user_guide` blocks 6, 7 and 9.
...
.. list-table::
   ...
   * - ``seed``
     ...
     - Random seed, used for both ``jax.random.PRNGKey`` and
       ...
       initialization and therefore the same reconstruction.
   * - ``initniter``
     - int
     - ``3000``
   ...
   * - ``niter``
     - int
     - ``10000``
     - Number of data training iterations. Indexed variants
...
Typical values are ``0``--``6`` for the time coordinate and ``0``--``4`` for the 
spatial coordinates. In ``example_dynamic_imaging.py`` the same list serves both 
networks: the 3D dynamic network receives ``tuple(h.nposenc)`` and the 2D static 
network receives ``tuple(h.nposenc[-2:])``, i.e. the spatial entries only.

Network Architecture
--------------------

See also :doc:`user_guide` block 6.
...
.. list-table::
   ...
   * - ``depth``
     ...
     - Number of hidden layers. In ``example_dynamic_imaging.py`` the static
       and dynamic networks are sized separately by ``s_depth`` and ``d_depth``; 
       the static network can be shallower because it represents a single 2D 
       image.
   * - ``width``
     - int
     - ``256``
   ...
   * - ``outshift``
     ...
     - ``10``
     - ``10``
     - Shift subtracted from the output nodes before the output activation,
       ``outactiv(x - outshift)``. It pushes the initial, near-zero values far
   ...
   * - ``scaling_ml``
     ...
     - Scaling of the linear polarization fraction channel,
       ...
       polarization the network can produce.
...
network, and ``nn.sigmoid`` for the normalized components of a decomposition). 
The EVPA channels use fixed sigmoids and are not configurable, and
